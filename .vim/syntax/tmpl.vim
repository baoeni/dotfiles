...
syn match htmltmpl_tagname contained +TMPL_\(IF\|ELSIF\|INCLUDE\|ELSE\|VAR\|UNLESS\|SETVAR\|LOOP\)+
syn keyword htmltmpl_loop_tagname contained TMPL_LOOP

syn include @perlTop runtime syntax/perl.vim
syn region tPerl matchgroup=Delimiter start="\[%" end="%\]" contains=@perlTop
...
" IF\|ELSIF\|INCLUDE\|ELSE\|VAR\|UNLESS\|SETVAR\|LOOP
if exists("loaded_matchit")
    let b:match_ignorecase = 1
    let b:match_words = '<TMPL_IF.*>:</TMPL_IF>,'
                        \. '<TMPL_UNLESS.*>:</TMPL_UNLESS>,'
                        \. '<TMPL_LOOP.*>:</TMPL_LOOP>,'
                        \. '<TMPL_SETVAR.*>:</TMPL_SETVAR>,'
                        \. '<TMPL_IF.*>:<TMPL_ELSE>:</TMPL_IF>,' 
                        \. '<TMPL_IF.*>:<TMPL_ELSIF.*>:<TMPL_ELSE>:</TMPL_IF>,' 
endif
